  ...
  output$my_leaf <- renderLeaflet({
    leaflet()%>%
      ...
      addPolylines(data = my_shp2,color = "red", 
                   weight  = 3, opacity = 1,
                   fillOpacity = 0,smoothFactor = 1, dashArray ="4",
      ...
      #setView(13.169629, 50.860422, zoom = 10)%>%
  ...
  observe({
    ...
    proxy<-leafletProxy(mapId = "my_leaf", data = df_filtered()) %>%
      ...
      addCircleMarkers(lng = green_df$lon,
                       lat = green_df$lat,
                       radius = 8,color = "black",weight = 2,opacity = 1,
                       stroke = TRUE,fillColor = "green",fillOpacity = 0.6,
                       popup = paste("<b><a href='",green_df$wiki_url,"'>",
                                     green_df$Name,"</a></b>","<br>","<img src = '",
                                     green_df$img_url, "'>"))
